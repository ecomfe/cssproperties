    ...
    var candidates = [
        {
            ...
            selector: '.col-3 a',
            callback: function () {
                allProps = util.extend(allProps, this.props);
                // console.warn(this);
        ...
        (function (index) {
            new Fetch({
                ...
                if (index === len - 1) {
                    ...
                    var jsonAllProps = JSON.stringify(allProps, null, 4);
                    var template = '{\n  "___AUTO_GENERATED___": "' + (new Date()) + '",\n';
                    template += '  "count": "' + allProps.length + '",\n';
                    template += '  "properties": ' + jsonAllProps.replace(/\]$/, '  ]') + '\n}';
                    fs.writeFileSync(path.join(__dirname, '../raw', path.sep) + 'all.json', template);
                    console.log('Update/build all.json complete successfully! :)');
                }
